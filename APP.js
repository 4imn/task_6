...
app.post("/bookshop", (req, res) => {
    ...
    connection.query(query, [ name,contactNumber, email, Address, city], (err) => {
        if (err) {
            return res.status(500).json({ error: "Error adding new book shop", details: err.message });
        }
        res.status(201).json({ message: "Book shop has been added" });
    });
});


// Get shop book by ID
app.get("/bookshop/id/:shop_id", (req, res) => {
    const query = "SELECT * FROM bookshop WHERE shop_id = ?";
    
    connection.query(query, [req.params.shop_id], (err, results) => {
        if (err) {
            return res.status(500).json({ error: "Error retrieving the book shop", details: err.message });
        }
        if (results.length === 0) {
            return res.status(404).json({ message: "Book shop not found" });
        }
        res.json(results[0]);
    });
});
...
app.put("/bookshop/name/:shop_id", (req, res) => {
    ...
    connection.query(query, [name, req.params.shop_id], (err, results) => {
        if (err) {
            return res.status(500).json({ error: "Error updating the book shop", details: err.message });
        }
        if (results.affectedRows === 0) {
            return res.status(404).json({ message: "Book shop not found" });
        }
        res.status(200).json({ message: "Book shop name has been updated" });
    });
});
...
app.put("/bookshop/email/:shop_id", (req, res) => {
    ...
    connection.query(query, [email, req.params.shop_id], (err, results) => {
        ...
        res.status(200).json({ message: "Book shop email has been updated" });
    });
});
